...
from model import PLE # 导入我们新的PLE模型

def evaluate_ple_model(model, dataloader, device):
    model.eval()
    all_labels = {task: [] for task in TASKS}
    all_preds = {task: [] for task in TASKS}

    with torch.no_grad():
        for batch in tqdm(dataloader, desc="正在验证"):
...
def main():
    ...
    model = PLE(
        ...
        cgc_hidden_units=HIDDEN_UNITS, # 使用PLE专属配置
    ...
    for epoch in range(EPOCHS):
        ...
        # --- 在每个epoch结束后，进行评估 ---
        test_auc_scores = evaluate_ple_model(model, test_loader, device)
        ...
        if avg_auc > best_avg_auc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVED_MODEL_PATH, "ple_best_model.pth"))
