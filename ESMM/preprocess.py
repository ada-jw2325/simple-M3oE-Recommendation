...
file_path = os.path.join(PROCESSED_DATA_PATH, 'feature_max_idx.json')
# 保存特征词汇表信息
with open(file_path, 'w') as f:
    json.dump(feature_max_idx, f)

# 同步导出一份不含 tagSize 的特征字典，供 MMOE/MTL/PLE 使用
feature_max_idx_without_tag = {k: v for k, v in feature_max_idx.items() if k != 'tagSize'}
file_path_without_tag = os.path.join(PROCESSED_DATA_PATH, 'feature_max_idx_withoutTag.json')
with open(file_path_without_tag, 'w') as f:
    json.dump(feature_max_idx_without_tag, f)
