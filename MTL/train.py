...
from model import SharedBottom # 导入SharedBottom模型

def evaluate_model(model, dataloader, device):
    """
    在训练过程中评估SharedBottom模型。
    Evaluates the SharedBottom model during training.
    """
    model.eval()
...
def main():
    ...
    # --- 2. 初始化模型、优化器、损失函数 ---
    print("正在初始化Shared-Bottom模型...")
    # 为SharedBottom模型定义网络结构
    BOTTOM_HIDDEN_UNITS = HIDDEN_UNITS['bottom']
    ...
    model = SharedBottom(
        ...
        bottom_hidden_units=BOTTOM_HIDDEN_UNITS, # 使用专属配置
        tower_hidden_units=TOWER_HIDDEN_UNITS,    # 使用专属配置
    ...
    for epoch in range(EPOCHS):
        ...
        # --- 在每个epoch结束后，进行评估 ---
        test_auc_scores = evaluate_model(model, test_loader, device)
        ...
        if avg_auc > best_avg_auc:
            ...
            torch.save(model.state_dict(), os.path.join(SAVED_MODEL_PATH , "shared_bottom_best_model.pth"))
            print(f"🎉 新的最优模型已保存! 平均AUC: {best_avg_auc:.4f}")

        scheduler.step()

    print(f"\n✅ 训练完成！最优模型已保存 (最优平均AUC: {best_avg_auc:.4f})")
